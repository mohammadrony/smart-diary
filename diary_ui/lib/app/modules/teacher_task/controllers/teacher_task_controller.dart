...
class TeacherTaskController extends GetxController {
  var task = Task().obs;
  var courseId = '';
  var todos = [].obs;
  var error = false;
  ...
  Future<void> getTeacherTask(String id) async {
    var apiResponse = await TaskService.getTeacherTask(id);
  ...
  Future<void> createTask(Task task) async {
    var apiResponse = await TaskService.createCourseTask(task);
    if (apiResponse.error == true) {
      error = true;
      errorMessage = apiResponse.errorMessage;
    } else {
  ...
  Future<void> updateTask(Task task) async {
    var apiResponse = await TaskService.updateTeacherTask(task);
    if (apiResponse.error == true) {
      error = true;
      errorMessage = apiResponse.errorMessage;
    }
  ...
  Future<void> deleteTask(String id) async {
    var apiResponse = await TaskService.deleteTeacherTask(id);
    if (apiResponse.error == true) {
      error = true;
      errorMessage = apiResponse.errorMessage;
    }
